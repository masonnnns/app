...
client:on("memberLeave", function(member)
  ...
  if config[member.guild.id].auditlog ~= "nil" and member.guild:getChannel(config[member.guild.id].auditlog) ~= nil then
    local roles = {}
    if cache[member.guild.id] ~= nil and cache[member.guild.id].users[member.id] ~= nil then for items,_ in pairs(cache[member.guild.id].users[member.id].roles) do roles[1+#roles] = member.guild:getRole(items).mentionString end end
