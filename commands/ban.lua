  ...
  if args[2] == nil then
    ...
    elseif message.guild:getMember(client.user.id):hasPermission("banMembers") ~= true then
      ...
      if args[3] == nil then
        ...
        data.modData.cases[1+#data.modData.cases] = {type = "ban", reason = reason, moderator = message.author.id, user = user.id, duration = "Permanent", case = 1+#data.modData.cases, id = 0}
      ...
      elseif durationTable[table.concat(duration.char,"")] == nil then
        ...
        if tonumber(table.concat(duration.numb,"")) * durationTable[table.concat(duration.char,"")][1] <= 0 then
          ...
          message.guild:banUser(user,reason,7)
          data.modData.cases[1+#data.modData.cases] = {type = "ban", reason = reason, moderator = message.author.id, user = user.id, duration = table.concat(duration.numb,"").." "..durationTable[table.concat(duration.char,"")][2]..(tonumber(table.concat(duration.numb,"")) == 1 and "" or "s"), id = 0}
          config.updateConfig(message.guild.id,data)
          if data.modlog ~= "nil" and message.guild:getChannel(data.modlog) ~= nil then
